...
func TestNIXL_MergeAndPrepare_DuplicateHashes_Identical(t *testing.T) {
	...
	c.MergeEncodeResponse(reqCtx, map[string]any{"hash-a": map[string]any{"peer_port": 5501, "nixl_agent_metadata_b64": "bml4..."}})
	c.MergeEncodeResponse(reqCtx, map[string]any{"hash-a": map[string]any{"peer_port": 5501, "nixl_agent_metadata_b64": "bml4..."}})
...
func TestNIXL_MergeEncodeResponse_NilInnerValue(t *testing.T) {
	...
	reqCtx := &pipeline.RequestContext{}
	c.MergeEncodeResponse(reqCtx, map[string]any{"hash-x": nil})
...
func TestNIXL_PreparePrefillECParams_Idempotent(t *testing.T) {
	...
	first, err := c.PreparePrefillECParams(reqCtx)
	if err != nil {
		t.Fatalf("PreparePrefillECParams: %v", err)
	}
	second, err := c.PreparePrefillECParams(reqCtx)
	if err != nil {
		t.Fatalf("PreparePrefillECParams: %v", err)
	}
